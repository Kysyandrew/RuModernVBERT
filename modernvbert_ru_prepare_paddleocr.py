...
TRANSFORMERS_COLMODERNVBERT_IMPORT_ERROR = "disabled in OCR preparation module"

ColPaliModernVBert = None
ColPaliModernVBertProcessor = None
COLPALI_MODERNVBERT_IMPORT_ERROR = "disabled in OCR preparation module"

try:
    from paddleocr import PaddleOCR
    PADDLEOCR_IMPORT_ERROR = None
except Exception as exc:
    PADDLEOCR_IMPORT_ERROR = f"{type(exc).__name__}: {exc}"
    PaddleOCR = None

easyocr = None
EASYOCR_IMPORT_ERROR = "disabled in PaddleOCR preparation module"
...
def environment_preflight(
    ...
    report: dict[str, Any] = {
        ...
        "import_errors_at_module_load": {
            ...
            "paddleocr": PADDLEOCR_IMPORT_ERROR,
            "easyocr": EASYOCR_IMPORT_ERROR,
        },
        "imports": {},
        "modes": {
    ...
    for module_name in ("torch", "transformers", "datasets", "PIL", "paddleocr", "paddle"):
        try:
...
def assert_v2_environment_ready(
    ...
    if require_ocr and not report["imports"].get("paddleocr", {}).get("ok", False):
        problems.append(
            "PaddleOCR is not importable. "
            f"Module-load error: {PADDLEOCR_IMPORT_ERROR}. "
            f"Reinstall with:\n{build_kaggle_paddleocr_reinstall_command()}"
        )
